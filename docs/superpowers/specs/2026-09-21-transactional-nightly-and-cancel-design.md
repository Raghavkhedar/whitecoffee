...
| **500-write cap** | Writes are ≤ N status `set`s + 1 `users` update + 1 leave update = **N + 2**. Reads are 1 + 2N. A leave's `totalDays` is bounded 1–366 by the rules and `retroLeaveScoring.js` uses `MAX_DAYS = 400` for the same reason. Worst case N = 366 → 368 writes, 733 reads: legal but slow. Recommend an explicit cap (§6 Q2). |
...
*Recommendation:* **200**, with a clear thrown message ("cancel in smaller batches"). Legal ceiling is ~366 (bounded by the rules' `totalDays`), but a 366-date transaction is 733 reads and a slow, contention-prone commit, and cancelling 200+ days in one action is not a real workflow — the UI picker is per-date.
