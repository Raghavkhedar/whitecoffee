...
**Auth guard & tab access**: `src/app/(admin)/layout.tsx` listens to `onAuthStateChanged`, redirects to `/login`, and admits a user via `hasPortalAccess` — `role === 'admin'` (superuser, all tabs) **or** a recognized `tags` entry on their `users` doc (scoped access). The layout also runs a central per-tab URL guard (`canAccess`) that redirects to the user's `landingPath` if they reach a tab they lack, so hand-typed URLs are blocked without per-page code. Access logic is pure/unit-tested in **`src/lib/portalAccess.ts`** (the single tab registry the Sidebar renders from, the tag→tabs map, and `allowedPaths`/`canAccess`/`landingPath`); the layout provides the user via `AccessContext` (`useAccess`). Tags are assigned by admins on the `/users` page. **Preset tags:** `attendance-manager` → Working Hours-Shortage/Excess, Attendance, OT & Shortage, OT Settlements, Manpower Utilisation Input. Firestore rules grant tagged users the matching read/write via `isAttendanceMgr()` (user-management writes stay admin-only — no self-promotion). Add a preset = one entry in `TAG_TABS`/`TAG_LABELS` + the matching rules grant.

**Routing**: `(admin)` is a route group (no URL segment) — `/dashboard`, `/working-hours-shortage-excess`, `/users`, `/leaves`, `/regularization`, `/attendance`, `/ot-shortage`, `/ot-settlements`, `/manpower-utilisation-input`, `/submissions`, `/conveyance`, `/notifications` are all protected.
...
- `users/{uid}/attendance/` — check-in/out events. Ops `site_in`/`site_out` carry free-text `siteName`; `siteId` is filled later by admin via **Manpower Utilisation Input** page (`updateAttendanceSiteId`).
